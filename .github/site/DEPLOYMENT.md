...
Run these commands from `.github/site/` before opening a deployment PR or pushing to `main`:
...
1. Commit the `.github/site/` project and `.github/workflows/deploy-site.yml`.
2. Push to `main`, or run the workflow manually with `workflow_dispatch`.
3. The workflow installs dependencies with `npm ci`, runs `npm run audit:build`, uploads `.github/site/dist`, and deploys the uploaded artifact to GitHub Pages.
