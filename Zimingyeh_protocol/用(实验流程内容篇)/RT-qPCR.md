...
**操作内容**：测定 RNA 浓度和纯度，必要时检测 [RNA完整性](<../番外/补充知识/RNA完整性.md>)。常用指标包括 [A260/A280](<../番外/补充知识/A260-A280.md>)、A260/A230、凝胶电泳或 RIN（RNA integrity number，RNA 完整性数值）。

**意义**：相同输入量和相近质量的 RNA 是样本之间可比的基础。[NanoDrop](<../材(实验耗材工具篇)/微量紫外分光光度计.md>) 这类分光法可以快速估算纯度，但不能准确判断 RNA 是否降解；荧光定量更适合低浓度 RNA。
...
- 对低浓度样本，用 [Qubit](<../材(实验耗材工具篇)/Qubit荧光计.md>) 等荧光法定量更可靠。
- 对高要求实验，用 [Bioanalyzer](<../材(实验耗材工具篇)/Bioanalyzer.md>)、[TapeStation](<../材(实验耗材工具篇)/TapeStation.md>) 或凝胶检查完整性。
